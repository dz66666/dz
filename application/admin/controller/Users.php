...
class Users extends Controller
{
    ...
    public function index()
    {
        ...
        $query->field('u.id,u.tel,u.username,u.lixibao_balance,u.is_manual,u.id_status,u.ip,u.is_jia,u.addtime,u.invite_code,u.freeze_balance,u.status,u.balance,u1.username as parent_name,u.withdrawal')
    ...
    public function index1()
    {
        ...
        $query->field('u.id,u.tel,u.username,u.lixibao_balance,u.id_status,u.is_manual,u.ip,u.is_jia,u.addtime,u.invite_code,u.freeze_balance,u.status,u.balance,u1.username as parent_name')
            ->leftJoin('xy_users u1','u.parent_id=u1.id')
            ->where($where)
            ->order('u.id desc')
            ->page();
    ...
    public function index2()
    {
        ...
        $query->field('u.id,u.tel,u.username,u.lixibao_balance,u.id_status,u.ip,u.is_jia,u.addtime,u.invite_code,u.freeze_balance,u.status,u.balance,u1.username as parent_name')
            ->leftJoin('xy_users u1','u.parent_id=u1.id')
            ->where($where)
            ->order('u.id desc')
            ->page();
    ...
    public function add_users()
    {
        if(request()->isPost()){
            ...
            $res = model('Users')->add_users($tel,$user_name,$pwd,$parent_id,$token,'',1);
            if($res['code']!==0){
    ...
    public function add_users1()
    {
        if(request()->isPost()){
            ...
            $parent_id= input('post.parent_id/d',0);
            $token = input('__token__',1);
            $res = model('Users')->add_users($tel,$user_name,$pwd,$parent_id,$token,'',0);
            if($res['code']!==0){
                return $this->error($res['info']);
            }
            return $this->success($res['info']);
    ...
    public function add_users2()
    {
        if(request()->isPost()){
            ...
            $parent_id= input('post.parent_id/d',0);
            $token = input('__token__',1);
            $res = model('Users')->add_users($tel,$user_name,$pwd,$parent_id,$token,'',2);
            if($res['code']!==0){
                return $this->error($res['info']);
            }
            return $this->success($res['info']);
    ...
    public function edit_users()
    {
        ...
        if(request()->isPost()){
            ...
            $res = model('Users')->edit_users($id,$tel,$user_name,$pwd,$parent_id,$balance,$freeze_balance,$token,$pwd2);
            $res2 = Db::table($this->table)->where('id',$id)->update(['deal_status'=>$deal_status,'level'=>$level,'type'=>$type]);
...
//            $show_tel2 = input('post.show_tel2/d',0);  //显示电话隐藏
     ...
     * @auth true
     */
    public function edit_users_status3()
    {
        $id = input('id/d',0);
        $status = input('status/d',0);
        if(!$id || !$status) return $this->error('参数错误');
        ...
        $res = Db::table($this->table)->where('id',$id)->update(['withdrawal'=>$status]);
    ...
    public function edit_users_is_manual()
    {
        $id = input('id/d',0);
        $is_manual = input('is_manual/d',0);
        if(!$id || !$is_manual) return $this->error('参数错误');

        $res = Db::table($this->table)->where('id',$id)->update(['is_manual'=>$is_manual]);

        if(!$res){
            echo '<pre>';
            var_dump($res,$is_manual,$_REQUEST);
            return $this->error('更新失败!');
        }
        return $this->success('更新成功');
    }
    ...
    public function edit_users_level()
    {
        if(request()->isPost()){
            ...
            $tiktok_num   = input('post.tiktok_num/s','');
