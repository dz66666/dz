...
class Base extends Controller
{
     ...
     * 检查交易状态
     */
    public function check_deal($cid='')
    {
       
        $uid = session('user_id');
        $uinfo = db('xy_users')->field('deal_status,status,balance,level,deal_count,deal_time,deal_reward_count dc,tiktok_count')->find($uid);
        if($uinfo['status']==2) return ['code'=>1,'info'=>'该账户已被禁用'];
        if($uinfo['deal_status']==0) return ['code'=>1,'info'=>'该账户交易功能已被冻结'];
        if($uinfo['deal_status']==3) return ['code'=>1,'info'=>'已进入匹配中，无法继续匹配！'];
        ...
        if($uinfo['deal_time']==strtotime(date('Y-m-d'))){
            ...
            //重置最后交易时间
            db('xy_users')->where('id',$uid)->update(['deal_time'=>strtotime(date('Y-m-d')),'deal_count'=>0,'tiktok_count'=>0,'recharge_num'=>0,'deposit_num'=>0]);
        }
