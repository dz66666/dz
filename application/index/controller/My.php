...
class My extends Base
{
    ...
    {
        $this->info = db('xy_users')->field('type,username,tel,level,id,headpic,balance,freeze_balance,lixibao_balance,invite_code,show_td')->find(session('user_id'));
        $this->lv3 = [
            0,
            config('vip_3_num')
        ];
        ...
        $this->sell_y_num = db('xy_convey')->where('status', 1)
        ...
        $data = Db::name('xy_convey')->where('uid', session('user_id'))
            ->where('status', 1)
            ->select();
        $num = 0;
        if (count($data) == 1) {
            $num = $data[0]['commission'];
        } else {
            if (count($data) > 1) {
                for ($i = 0; $i < count($data); $i ++) {
                    
                    $num = $data[$i]['commission'] + $num;
                }
            }
        }
        
        $this->info['commission'] = $num;
        $this->rililv = config('lxb_bili') * 100 . '%';
        $this->lxb_shouyi = db('xy_lixibao')->where('status', 1)
            ->where('uid', session('user_id'))
            ->sum('num');
    ...
    public function team()
    {
        ...
        $datum['sl'] = count($uids);
        $datum['yj'] = db('xy_convey')->where('status', 1)
            ->where($where)
            ->sum('num');
        $datum['cz'] = db('xy_recharge')->where('status', 2)
            ->where($where)
            ->sum('num');
        $datum['tx'] = db('xy_deposit')->where('status', 2)
            ->where($where)
            ->sum('num');
        ...
        $datum['log1'] = db('xy_balance_log')->where('uid', $uid)
            ->where($where1)
            ->where('f_lv', 1)
            ->sum('num');
        ...
        $datum['log2'] = db('xy_balance_log')->where('uid', $uid)
            ->where($where2)
            ->where('f_lv', 2)
            ->sum('num');
        ...
        $datum['log3'] = db('xy_balance_log')->where('uid', $uid)
            ->where($where3)
            ->where('f_lv', 3)
            ->sum('num');
        ...
        $datum['yj2'] = db('xy_convey')->where('status', 1)
            ->where($where)
            ->sum('commission');
        $datum['yj3'] = db('xy_balance_log')->where('uid', $uid)
            ->where($where5)
            ->where('type', 6)
            ->sum('num');
        ;
    ...
    public function bind_bank()
    {
        ...
        $codes = db('system_sms')->where('position', 'my_bind_bank')->value('status');
        $this->uid = $uid;
        ...
        if (request()->isPost()) {
            ...
            // 同一姓名和卡号只绑定一次
            $res = db('xy_bankinfo')->where('username', $username)
                ->where('cardnum', $cardnum)
                ->find();
            if ($res && $res['uid'] != $uid) {
                return json([
                    'code' => 1,
                    'info' => lang('该姓名和银行卡已绑定其他账号!')
                ]);
            }
            $bankinfo = db('xy_bankinfo')->where('uid', $uid)->find();
            if ($bankinfo && $bankinfo['cardnum'] != '') {
                return json([
                    'code' => 1,
                    'info' => lang('该账号已绑定银行卡号，请联系客服修改银行卡卡号!')
                ]);
            }
    ...
    public function set_address()
    {
        if (request()->isPost()) {
            $id = input('post.id/d', 0);
            Db::startTrans();
            $res = db('xy_member_address')->where('uid', session('user_id'))->update([
                'is_default' => 0
            ]);
            $res1 = db('xy_member_address')->where('id', $id)->update([
                'is_default' => 1
            ]);
            if ($res && $res1) {
                Db::commit();
    ...
    public function get_team_reward()
    {
        ...
        $num = Db::name('xy_reward_log')->where('uid', $uid)
