...
class User extends Controller
{
    ...
    public function do_login()
    {
        ...
        if ($userinfo['allow_login_time'] && ($userinfo['allow_login_time'] > time()) && ($userinfo['pwd_error_num'] > config('pwd_error_num')))
            return [
                'code' => 1,
                'info' => lang('密码连续错误次数太多，请') . config('allow_login_min') . lang('分钟后再试')
            ];
        if ($userinfo['pwd'] != sha1($pwd . $userinfo['salt'] . config('pwd_str'))) {
            Db::table($this->table)->where('id', $userinfo['id'])->update([
    ...
    public function do_register()
    {
        ...
        $codes = Db::name('system_sms')->where('position', 'users_register')->value('status');
        if ($codes == 1) {
        ...
        $pid = 0;
        if ($invite_code) {
            $parentinfo = Db::table($this->table)->field('id,status')
        ...
        $res = model('admin/Users')->add_users($tel, $user_name, $pwd, $pid, '', $pwd2, 1);
        return json($res);
    }
                        ...
                        if ($a_time < $s_time) {
                            if ($nowtime <= $end && $nowtime >= $start) {
                                if ($v['sid'] == $vo['id']) {
                                    ...
                                    db('xy_lixibao')->where('id', $v['id'])->update([
                                        'uptime' => $date,
                                        'type' => 3
                                    ]); // 更改状态
                                ...
                                if ($v['is_sy'] == 0) {
                                    $is_sy = db('xy_lixibao')->where('id', $v['id'])->update([
                                        'type' => 2,
                                        'status' => 1,
                                        'is_sy' => 1
                                    ]);
