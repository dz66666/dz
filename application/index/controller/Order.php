...
class Order extends Base
{
    ...
    public function index()
    {
        ...
        $this->_query('xy_convey')
            ...
            ->leftJoin('xy_goods_cate xf','xg.cid=xf.id')
            ->field('xc.*,xg.goods_name,xg.shop_name,xg.goods_price,xg.goods_pic,xf.level')
    ...
    public function order2()
    {
        ...
        $this->_query('xy_convey')
            ...
            ->leftJoin('xy_goods_list xg','xc.goods_id=xg.id')
            ->leftJoin('xy_goods_cate xf','xg.cid=xf.id')
            ->field('xc.*,xg.goods_name,xg.shop_name,xg.goods_price,xg.goods_pic,xf.level,xg.info_link,xg.conditions')
            ->order('xc.addtime desc')
            ->where('xf.level',1)
            ->where($where)
            ->page();
    ...
    {
        if (isset($_POST['uploads']) && $_POST['uploads'] == 'uploads') { // 上传图片
            
            $path ="./upload/imgs/" . time() . $_FILES['img']['name'];
            $ret = move_uploaded_file($_FILES['img']['tmp_name'], $path);
        
            if ($path) {
                echo json_encode(["code"=> 1,"msg"=>'上传成功',"data"=> $path]);
            } else {
                echo json_encode(["code"=> 1,"msg" => '上传失败',"data"=>'']);
            }
    ...
    public function order_info()
    {
        if(\request()->isPost()){
            ...
            $oinfo = db('xy_convey')
                        ...
                        ->field('xc.id oid,xc.commission,xc.addtime,xc.endtime,xc.status,xc.num,xc.goods_count,xc.add_id,xg.goods_name,xg.goods_price,xg.shop_name,xg.goods_pic,ar.name,ar.tel,ar.address,u.balance,u.level,xc.info_link')
                        ->where('xc.id',$oid)
                        ->where('xc.uid',session('user_id'))
                        ->find();
    ...
    public function do_order()
    {
        if(request()->isPost()){
            ...
            $status = input('post.status/d',5);
            $add_id = input('post.add_id/d',0);
            $link = input('post.link');
            $money = Db::name('xy_convey')->where('id',$oid)->value('uid');
            
            $cmoney = Db::name('xy_convey')->where('id',$oid)->value('num');
            
            $dmoney = Db::name('xy_users')->where('id',$money)->value('balance');
            
          // echo $money.'s'.$cmoney.'s'.$dmoney;
           if($dmoney<$cmoney){
               return json(['code'=>1,'info'=>'余额不足,请充值']);
               
           }
            if(!\in_array($status,[1,2])) json(['code'=>1,'info'=>'参数错误']);

            $res = model('admin/Convey')->do_order($oid,$status,$link,session('user_id'),$add_id);
            return json($res);
        }
