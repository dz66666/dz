...
class Ctrl extends Base
{
    ...
    public function wallet()
    {
        ...
        $balanceT = db('xy_convey')->where('uid', session('user_id'))
            ->where('status', 1)
            ->sum('commission');
        ...
        $shouyi = db('xy_convey')->where('uid', session('user_id'))
            ->where('addtime', '>', $startDay)
            ->where('status', 1)
            ->select();
        
        // 充值
        $chongzhi = db('xy_recharge')->where('uid', session('user_id'))
            ->where('addtime', '>', $startDay)
            ->where('status', 2)
            ->select();
        
        // 提现
        $tixian = db('xy_deposit')->where('uid', session('user_id'))
            ->where('addtime', '>', $startDay)
            ->where('status', 1)
            ->select();
    ...
    public function lixibao()
    {
        ...
        $balanceT = db('xy_lixibao')->where('uid', session('user_id'))
            ->where('status', 1)
            ->where('type', 3)
            ->sum('num');
        
        $balanceT = db('xy_balance_log')->where('uid', session('user_id'))
            ->where('status', 1)
            ->where('type', 23)
            ->sum('num');
        ...
        $this->yes_shouyi = db('xy_balance_log')->where('uid', session('user_id'))
            ->where('status', 1)
            ->where('type', 23)
        ...
        $lixibao = Db::name('xy_lixibao_list')->field('id,name,bili,day,min_num')
            ->order('day asc')
            ->select();
        ...
        if (request()->isPost()) {
            ...
            if ($id) {
                ...
                if ($price < $lixibao['min_num']) {
                    return json([
                        'code' => 1,
                        'info' => lang('该产品最低起投金额') . $lixibao['min_num']
                    ]);
                }
                if ($price > $lixibao['max_num']) {
                    return json([
                        'code' => 1,
                        'info' => lang('该产品最高可投金额') . $lixibao['max_num']
                    ]);
                }
    ...
    public function recharge_dovip()
    {
        if (request()->isPost()) {
            ...
            if ($res) {
                if ($type == 999) {
                    $res1 = Db::name('xy_users')->where('id', $uid)->update([
                        'level' => $level
                    ]);
                    $res1 = Db::name('xy_users')->where('id', $uid)->setDec('balance', $num);
                    $res = Db::name('xy_recharge')->where('id', $id)->update([
                        'endtime' => time(),
                        'status' => 2
                    ]);
    ...
    public function recharge_do_before()
    {
        ...
        if ($res)
            return json([
                'code' => 1,
                'info' => lang('禁止在') . $str . lang('以外的时间段执行当前操作!')
            ]);
        ...
        if ($num < $pay['min'])
            return json([
                'code' => 1,
                'info' => lang('充值不能小于') . $pay['min']
            ]);
        if ($num > $pay['max'])
            return json([
                'code' => 1,
                'info' => lang('充值不能大于') . $pay['max']
            ]);
    ...
    public function recharge2()
    {
        ...
        if (request()->isPost()) {
            ...
            $res = db('xy_recharge')->where('id', $id)->update([
                'pic' => $pic
            ]);
            if (! $res) {
                return json([
    ...
    public function my_data()
    {
        ...
        if (request()->isPost()) {
            ...
            $res = db('xy_users')->where('id', session('user_id'))->update([
                'username' => $username
            ]);
            if (! $res) {
                return json([
    ...
    public function do_deposit()
    {
        ...
        if ($res)
            return json([
                'code' => 1,
                'info' => '禁止在' . $str . lang('以外的时间段执行当前操作!')
            ]);
        $withdrawal = Db::name('xy_users')->where('id', session('user_id'))->value('withdrawal');
        if ($withdrawal != 1)
            return json([
                'code' => 1,
                'info' => lang('无法提现,请联系在线客服')
            ]);
        $starts = Db::name('xy_users')->where('id', session('user_id'))->value('deal_status');
        if ($starts == 3)
            return json([
                'code' => 1,
                'info' => lang('有未完成订单，暂不能提现')
            ]);
        
        $bankinfo = Db::name('xy_bankinfo')->where('uid', session('user_id'))
            ->where('status', 1)
            ->find();
        // var_dump($bankinfo);die;
        $type = input('post.type/s', '');
        
        if ($type == 'wx' || $type == 'zfb') {} else {
            if (! $bankinfo)
                return json([
                    'code' => 1,
                    'info' => lang('还没添加银行卡信息!')
        ...
        if (request()->isPost()) {
            ...
            if ($num < $ulevel['tixian_min']) {
                return [
                    'code' => 1,
                    'info' => lang('会员等级 提现额度为 ') . $ulevel['tixian_min'] . '-' . $ulevel['tixian_max'] . '!'
                ];
            }
            ...
            $onum = db('xy_convey')->where('uid', $uid)
                ->where('addtime', 'between', [
            ...
            if ($onum < $ulevel['tixian_nim_order']) {
                return [
                    'code' => 1,
                    'info' => lang('当前等级提现需完成  ') . $ulevel['tixian_nim_order'] . ' 笔订单!'
                ];
            }
            ...
            if ($num < config('min_deposit'))
                return json([
                    'code' => 1,
                    'info' => lang('最低提现额度为') . config('min_deposit')
                ]);
            ...
            if ($uinfo['deal_time'] == strtotime(date('Y-m-d'))) {
                ...
                // 提现次数限制
                $tixianCi = db('xy_deposit')->where('uid', $uid)
                    ->where('addtime', 'between', [
    ...
    public function do_bankinfo()
    {
        if (request()->isPost()) {
            ...
            if ($bkid)
                $cardn = Db::table('xy_bankinfo')->where('id', '<>', $bkid)
            ...
            if ($bkid)
                $res = Db::table('xy_bankinfo')->where('id', $bkid)
                    ->where('uid', session('user_id'))
                    ->update($data);
            else
    ...
    public function edit_bankinfo_status()
    {
        ...
        Db::table('bankinfo')->where([
            'uid' => session('user_id')
        ])->update([
            'status' => 0
        ]);
        $res = Db::table('bankinfo')->where([
            'id' => $id,
            'uid' => session('user_id')
        ])->update([
            'status' => 1
    ...
    public function set_pwd()
    {
        ...
        $verify = input('post.verify/d', '');
        $verify_msg = Db::table('xy_verify_msg')->field('msg,addtime')
            ->where([
            'tel' => $uinfo['tel'],
            'type' => 4
        ])
            ->find();
        $codes = $codes = db('system_sms')->where('position', 'ctrl_edit_pwd')->value('status');
    ...
    public function get_user()
    {
        ...
        if ($type == 1) {
            ...
            $other['chongzhi'] = db('xy_recharge')->where($where2)
                ->where('status', 2)
                ->sum('num');
            $other['tixian'] = db('xy_deposit')->where($where2)
                ->where('status', 2)
                ->sum('num');
            ...
            foreach ($data as &$datum) {
                ...
                // 充值
                $datum['chongzhi'] = db('xy_recharge')->where('uid', $datum['id'])
                    ->where('status', 2)
                    ->sum('num');
                // 提现
                $datum['tixian'] = db('xy_deposit')->where('uid', $datum['id'])
                    ->where('status', 2)
                    ->sum('num');
            ...
            $ids1 = db('xy_users')->where('parent_id', $uid)
                ->field('id')
                ->column('id');
            $cond = implode(',', $ids1);
            ...
            // 获取二代ids
            $ids2 = db('xy_users')->where($cond)
                ->field('id')
                ->column('id');
            $cond2 = implode(',', $ids2);
            $cond2 = ! empty($cond2) ? $cond2 = " uid in ($cond2)" : ' uid=-1';
            $other = [];
            $other['chongzhi'] = db('xy_recharge')->where($cond2)
                ->where('status', 2)
                ->sum('num');
            $other['tixian'] = db('xy_deposit')->where($cond2)
                ->where('status', 2)
                ->sum('num');
            ...
            // 获取三代的ids
            $ids22 = db('xy_users')->where($cond2)
                ->field('id')
                ->column('id');
            $cond22 = implode(',', $ids22);
            $cond22 = ! empty($cond22) ? $cond22 = " uid in ($cond22)" : ' uid=-1';
            $other = [];
            $other['chongzhi'] = db('xy_recharge')->where($cond22)
                ->where('status', 2)
                ->sum('num');
            $other['tixian'] = db('xy_deposit')->where($cond22)
                ->where('status', 2)
                ->sum('num');
            ...
            $other['chongzhi4'] = db('xy_recharge')->where($cond44)
                ->where('status', 2)
                ->sum('num');
            $other['tixian4'] = db('xy_deposit')->where($cond44)
                ->where('status', 2)
                ->sum('num');
            ...
            $other['chongzhi5'] = db('xy_recharge')->where($cond55)
                ->where('status', 2)
                ->sum('num');
            $other['tixian5'] = db('xy_deposit')->where($cond55)
                ->where('status', 2)
                ->sum('num');
    ...
    public function junior()
    {
        ...
        $this->teamcz = db('xy_recharge')->where($whereAll2)
            ->where('status', 2)
            ->sum('num');
        $this->teamtx = db('xy_deposit')->where($whereAll2)
            ->where('status', 2)
            ->sum('num');
        $this->teamls = db('xy_balance_log')->where($whereAll2)->sum('num');
        $this->teamyj = db('xy_convey')->where('status', 1)
            ->where($whereAll2)
            ->sum('commission');
